...
class DomainAdversarialLoss(nn.Module):
    ...
    def forward(self, f_s: torch.Tensor, f_t: torch.Tensor,
        ...
        # 标签定义（保持您的原有设置）
        d_label_s = torch.ones((f_s.size(0), 1)).to(f_s.device)  # 源域为1
        d_label_t = torch.zeros((f_t.size(0), 1)).to(f_t.device)  # 目标域为0

        self.domain_discriminator_accuracy = 0.5 * (binary_accuracy(d_s, d_label_s) + binary_accuracy(d_t, d_label_t))

        # 处理权重 - 确保没有梯度且形状正确
        if w_s is None:
            w_s = torch.ones(f_s.size(0), 1, device=f_s.device)
        else:
            w_s = w_s.detach()
            # 确保权重形状为 [batch_size, 1]
            if w_s.dim() == 1:
                w_s = w_s.unsqueeze(1)

        if w_t is None:
            w_t = torch.ones(f_t.size(0), 1, device=f_t.device)
        else:
            w_t = w_t.detach()
            # 确保权重形状为 [batch_size, 1]
            if w_t.dim() == 1:
        ...
        return 0.5 * (self.bce(d_s, d_label_s, w_s) + self.bce(d_t, d_label_t, w_t))
